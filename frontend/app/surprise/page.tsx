...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p
      ...
      style={{ color: "#A87A8A", fontFamily: "var(--font-inter)", fontWeight: 500 }}
    >
      {children}
    </p>
  );
...
function HeroSection({ heroSrc }: { heroSrc?: string | null }) {
  ...
  return (
    ...
    >
      <motion.div style={{ y, opacity }} className="flex flex-col items-center gap-6 w-full max-w-xl">
        ...
        {/* Message card */}
        <motion.div
          className="glass-warm shadow-warm-xl w-full rounded-3xl px-7 py-6 text-left"
...
function LetterModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <motion.div
        className="relative w-full max-w-4xl rounded-[32px] bg-white/95 border border-slate-200 shadow-[0_40px_120px_rgba(15,23,42,0.18)] overflow-hidden"
      ...
      >
        ...
        <div className="px-8 py-8 sm:px-12 sm:py-10">
          <div className="mx-auto max-w-3xl text-center">
            <h3 className="text-4xl font-semibold tracking-tight text-slate-900">{LETTER.heading}</h3>
            <p className="mt-3 text-sm uppercase tracking-[0.26em] text-slate-500">Written with love</p>
          </div>

          <div className="mt-10 max-h-[70vh] overflow-y-auto rounded-3xl border border-slate-200 bg-slate-50 p-8 text-left text-lg leading-8 tracking-[0.01em] text-slate-900 shadow-inner">
            {LETTER.body.split("\n\n").map((para, i) => (
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        {item.caption && (
          <p className="mt-4 text-sm text-center" style={{ color: "rgba(250,246,241,0.75)", fontFamily: "var(--font-lora)", fontStyle: "italic" }}>
            {item.caption}
          </p>
        )}
        {/* Counter */}
        <p className="mt-2 text-xs tracking-widest" style={{ color: "rgba(255,255,255,0.35)", fontFamily: "var(--font-inter)" }}>
...
function GalleryCard({ item, index, onClick }: { item: GalleryItem; index: number; onClick: () => void }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: 1.06, rotate: 1.5, boxShadow: "0 20px 50px rgba(200,90,90,0.3)" }}
      onClick={onClick}
    >
      <img
        src={item.src}
      ...
      <div
        ...
        style={{ background: "linear-gradient(to top, rgba(200,90,90,0.6) 0%, transparent 60%)" }}
      >
        {item.caption && (
          <p className="text-white text-[10px] leading-snug" style={{ fontFamily: "var(--font-lora)", fontStyle: "italic" }}>
            {item.caption}
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {HEARTFELT_NOTES.map((note, i) => (
          <RevealSection key={note.id} delay={i * 0.08} direction="scale">
            <motion.div
              className="glass-warm rounded-2xl p-6 shadow-warm text-center h-full"
          ...
          <RevealSection key={reason.id} delay={i * 0.07} direction="scale">
            ...
            >
              ...
              <h3
                ...
                style={{ color: "#7A4A5A", fontFamily: "var(--font-playfair)" }}
              >
                {reason.quality}
              </h3>
              <p
...
function FinalMessage() {
  return (
    <section className="px-4 w-full max-w-4xl mx-auto flex flex-col items-center text-center" style={{ paddingTop: "140px", paddingBottom: "140px" }}>
      <RevealSection className="w-full">
        ...
        {/* Message glass card */}
        <div
          className="glass-warm rounded-3xl px-8 py-8 shadow-warm-xl mb-10 text-left"
          style={{ border: "1px solid rgba(255,255,255,0.9)" }}
        >
          {FINAL_MESSAGE.body.split("\n\n").map((para, i) => (
            <p
              key={i}
              className={`text-[15px] leading-[1.95] ${i > 0 ? "mt-5" : ""}`}
              style={{
                color: "#7A4A5A",
                fontFamily: "var(--font-lora), 'Lora', Georgia, serif",
                fontStyle: "italic",
...
export default function SurprisePage() {
  ...
  const candidateHeroSrc = manifest?.hero ?? HERO.profileImage;
  const heroSrc = usePreloadImage(candidateHeroSrc);
      ...
      <main className="relative z-10 flex flex-col items-center w-full">
        {heroSrc ? (
          ...
          <section
            ...
            style={{ paddingTop: "5vh", paddingBottom: "10vh", opacity: 0 }}
            aria-hidden="true"
          >
